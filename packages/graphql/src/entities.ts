...
export function getAllJsonObjects(
  ...
  return Object.values(getSchema(_schema).getTypeMap())
    .filter((node) =>
      node.astNode?.directives?.find(
        ({ name: { value } }) => value === DirectiveName.JsonField
      )
    )
    .map((node) => node)
    .filter(isObjectType);
...
export function getAllEntitiesRelations(
  _schema: GraphQLSchema | string
): GraphQLModelsRelationsEnums {
  ...
  const entities = Object.values(schema.getTypeMap())
    .filter((node) =>
      node.astNode?.directives?.find(
        ({ name: { value } }) => value === DirectiveName.Entity
      )
    )
    .filter(isObjectType);
  ...
  const enums = new Map(
    getEnumsFromSchema(schema).map((node) => [
      node.name,
      {
        ...
        values: node.getValues().map((v) => v.value),
      },
    ])
  );
  ...
  const modelRelations = {
    ...
    relations: [],
    enums: [...enums.values()],
  } as GraphQLModelsRelationsEnums;
  ...
  for (const entity of entities) {
    ...
    for (const field of Object.values(entity.getFields())) {
      ...
      const derivedFromDirectValues = getDirectiveValues(
        derivedFrom,
        field.astNode
      );
      const indexDirectiveVal = getDirectiveValues(
        indexDirective,
        field.astNode
      );
      ...
      // If is an enum
        ...
        newModel.fields.push({
          ...
          isEnum: true,
          isArray: isListType(
            isNonNullType(field.type) ? getNullableType(field.type) : field.type
          ),
      ...
      if (indexDirectiveVal) {
        if (typeString !== "ID" && typeClass) {
          newModel.indexes.push({
            unique: indexDirectiveVal.unique as boolean,
            fields: [field.name],
          });
          ...
          if (indexDirectiveVal.unique) {
            const fkIndex = newModel.indexes.find(
              (idx) =>
                idx.fields.length === 1 && idx.fields[0] === `${field.name}Id`
            );
            if (fkIndex) {
              fkIndex.unique = true;
            }
...
function extractType(type: GraphQLOutputType): string {
  ...
  const offListType = isListType(offNullType)
    ? assertListType(offNullType).ofType
    : type;
...
function validateRelations(modelRelations: GraphQLModelsRelationsEnums): void {
  for (const r of modelRelations.relations.filter(
    (model) => model.type === "hasMany" || model.type === "hasOne"
  )) {
    assert(
